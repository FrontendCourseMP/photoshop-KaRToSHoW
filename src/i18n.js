import i18n from 'i18next'
import { initReactI18next } from 'react-i18next'

// Выбираем язык из localStorage или по умолчанию русский
const language = localStorage.getItem('language') || 'ru'
// Переводы для английского и русского интерфейсов
const resources = {
  en: {
    translation: {
      menu: {
        file: 'File',
        view: 'View',
        settings: 'Settings',
        open: 'Open…',
        exportPng: 'Export as PNG',
        exportJpeg: 'Export as JPEG',
        exportGb7: 'Export as GB7',
        zoomIn: 'Zoom In',
        zoomOut: 'Zoom Out',
        fitScreen: 'Fit to Screen',
        actualSize: 'Actual Size',
        accentColor: 'Accent color…',
        themeSettings: 'Theme…',
        themeSettingsSubtitle: 'Pick light or dark mode',
        theme: 'Theme',
        themeLight: 'Light',
        themeDark: 'Dark',
        chooseAccent: 'Pick accent color',
        chooseLanguage: 'Pick the app language',
        close: 'Close',
        language: 'Language',
        languageEnglish: 'English',
        languageRussian: 'Русский',
        filters: 'Filters',
      },
      toolbar: {
        zoomOutTitle: 'Zoom out',
        zoomInTitle: 'Zoom in',
        zoomToolTitle: 'Zoom tool',
        eyedropperTitle: 'Eyedropper',
        eyedropperSample: 'Sample color',
        handPanTitle: 'Hand / Pan',
        fitTitle: 'Fit to screen',
        fillTitle: 'Fill screen',
        actualTitle: 'Actual size',
        fit: 'Fit',
        fill: 'Fill',
        actual: '1:1',
      },
      info: {
        image: 'Image',
        eyedropper: 'Eyedropper',
        eyedropperEmpty: 'Select Eyedropper and click on image',
        copyHex: 'Copy HEX',
        width: 'Width',
        height: 'Height',
        depth: 'Depth',
        format: 'Format',
        pixels: 'Pixels',
        view: 'View',
        zoom: 'Zoom',
        displaySize: 'On screen',
        tool: 'Tool',
        shortcuts: 'Shortcuts',
        scroll: 'Scroll',
        pan: 'Space+drag',
        zoomStep: 'Zoom step',
        fit: 'Fit',
        actual: '100%',
        hand: 'Hand',
        zoomTool: 'Zoom tool',
      },
      channels: {
        title: 'Channels',
        red: 'Red',
        green: 'Green',
        blue: 'Blue',
        alpha: 'Alpha',
        mask: 'Mask',
        grayscale: 'Grayscale',
        active: 'active',
        noImage: 'No image loaded',
      },
      empty: {
        title: 'No image open',
        sub: 'Drag a file here to open',
        formats: 'PNG · JPEG · GB7',
        drop: 'Drop to open',
      },
      error: {
        cannotLoad: 'Cannot load image.',
        unsupported: 'Unsupported format. Use PNG, JPEG or GB7.',
        close: '✕',
      },
      status: {
        noFile: 'No file open',
      },
      toolPlaceholder: 'Coming soon',
      resize: {
        title: 'Scale Image',
        pixelsBefore: 'Before',
        pixelsAfter: 'After',
        mp: 'Mpx',
        kp: 'Kpx',
        unit: 'Unit',
        unitPx: 'Pixels',
        width: 'Width',
        height: 'Height',
        lockAspect: 'Lock aspect ratio',
        method: 'Interpolation',
        bilinear: 'Bilinear',
        bilinearTip: 'Weighted average of 4 nearest pixels. Smooth result with no sharp edges. Best for photos and natural images.',
        nearest: 'Nearest Neighbor',
        nearestTip: 'Picks the nearest pixel without blending. Very fast, preserves sharp edges. May produce a blocky result on upscaling.',
        open: 'Scale Image…',
      },
      levels: {
        title: 'Levels',
        channel: 'Channel',
        master: 'Master',
        histogram: 'Histogram',
        linear: 'Linear',
        logarithmic: 'Log',
        inputLevels: 'Input Levels',
        black: 'Black',
        gamma: 'Gamma',
        white: 'White',
        preview: 'Preview',
        reset: 'Reset',
        cancel: 'Cancel',
        apply: 'Apply',
        open: 'Adjust Levels…',
      },
      rp: {
        tabLevels: 'Levels',
        tabResize: 'Scale',
        tabFilter: 'Filter',
        levelsDesc: 'Adjust black point, gamma and white point per channel.',
        resizeDesc: 'Scale the image using bilinear or nearest-neighbor interpolation.',
        filterDesc: 'Apply a 3×3 convolution kernel to one or all channels.',
      },
      kernel: {
        title: 'Custom Filter',
        preset: 'Preset',
        matrix: 'Kernel Matrix',
        channels: 'Apply to Channels',
        edgeHandling: 'Edge Handling',
        processing: 'Processing…',
        custom: 'Custom',
        open: 'Custom Filter…',
        presets: {
          identity: 'Identity',
          sharpen: 'Sharpen',
          gaussian: 'Gaussian Blur (3×3)',
          boxBlur: 'Box Blur',
          prewittX: 'Prewitt X (Horizontal)',
          prewittY: 'Prewitt Y (Vertical)',
        },
        edge: {
          black: 'Fill Black',
          white: 'Fill White',
          copy: 'Copy Edge',
        },
      },
    },
  },
  ru: {
    translation: {
      menu: {
        file: 'Файл',
        view: 'Вид',
        settings: 'Настройки',
        open: 'Открыть…',
        exportPng: 'Экспорт в PNG',
        exportJpeg: 'Экспорт в JPEG',
        exportGb7: 'Экспорт в GB7',
        zoomIn: 'Увеличить',
        zoomOut: 'Уменьшить',
        fitScreen: 'Подогнать по экрану',
        actualSize: 'Реальный размер',
        accentColor: 'Цвет акцента…',
        themeSettings: 'Настройки',
        themeSettingsSubtitle: 'Выберите светлую или тёмную тему',
        theme: 'Тема',
        themeLight: 'Светлая',
        themeDark: 'Тёмная',
        chooseAccent: 'Выберите цвет акцента',
        chooseLanguage: 'Выберите язык приложения',
        close: 'Закрыть',
        language: 'Язык',
        languageEnglish: 'English',
        languageRussian: 'Русский',
        filters: 'Фильтры',
      },
      toolbar: {
        zoomOutTitle: 'Уменьшить',
        zoomInTitle: 'Увеличить',
        zoomToolTitle: 'Масштаб',
        eyedropperTitle: 'Пипетка',
        eyedropperSample: 'Выбрать цвет',
        handPanTitle: 'Рука / Перемещение',
        fitTitle: 'Подогнать по экрану',
        fillTitle: 'Заполнить экран',
        actualTitle: 'Реальный размер',
        fit: 'Подогнать',
        fill: 'Заполнить',
        actual: '1:1',
      },
      info: {
        image: 'Изображение',
        eyedropper: 'Пипетка',
        eyedropperEmpty: 'Выберите пипетку и кликните по изображению',
        copyHex: 'Копировать HEX',
        width: 'Ширина',
        height: 'Высота',
        depth: 'Глубина',
        format: 'Формат',
        pixels: 'Пикселей',
        view: 'Вид',
        zoom: 'Масштаб',
        displaySize: 'На экране',
        tool: 'Инструмент',
        shortcuts: 'Сочетания',
        scroll: 'Прокрутка',
        pan: 'Пробел + перетаскивание',
        zoomStep: 'Шаг масштаба',
        fit: 'По экрану',
        actual: '100%',
        hand: 'Рука',
        zoomTool: 'Масштаб',
      },
      channels: {
        title: 'Каналы',
        red: 'Красный',
        green: 'Зелёный',
        blue: 'Синий',
        alpha: 'Альфа',
        mask: 'Маска',
        grayscale: 'Оттенки серого',
        active: 'активен',
        noImage: 'Файл не загружен',
      },
      empty: {
        title: 'Изображение не открыто',
        sub: 'Перетащите файл сюда, чтобы открыть',
        formats: 'PNG · JPEG · GB7',
        drop: 'Отпустите, чтобы открыть',
      },
      error: {
        cannotLoad: 'Не удалось загрузить изображение.',
        unsupported: 'Неподдерживаемый формат. Используйте PNG, JPEG или GB7.',
        close: '✕',
      },
      status: {
        noFile: 'Файл не открыт',
      },
      toolPlaceholder: 'Скоро появится',
      resize: {
        title: 'Масштаб изображения',
        pixelsBefore: 'До',
        pixelsAfter: 'После',
        mp: 'Мпкс',
        kp: 'Кпкс',
        unit: 'Единица',
        unitPx: 'Пиксели',
        width: 'Ширина',
        height: 'Высота',
        lockAspect: 'Сохранить пропорции',
        method: 'Интерполяция',
        bilinear: 'Билинейная',
        bilinearTip: 'Взвешенное среднее 4 ближайших пикселей. Гладкий результат без резких краёв. Лучший выбор для фотографий.',
        nearest: 'Ближайший сосед',
        nearestTip: 'Берёт ближайший пиксель без смешения. Очень быстрый, сохраняет чёткие края. При увеличении может давать "пиксельный" эффект.',
        open: 'Масштаб изображения…',
      },
      levels: {
        title: 'Уровни',
        channel: 'Канал',
        master: 'Композит',
        histogram: 'Гистограмма',
        linear: 'Линейная',
        logarithmic: 'Лог',
        inputLevels: 'Входные уровни',
        black: 'Чёрный',
        gamma: 'Гамма',
        white: 'Белый',
        preview: 'Предпросмотр',
        reset: 'Сброс',
        cancel: 'Отмена',
        apply: 'Применить',
        open: 'Настроить уровни…',
      },
      rp: {
        tabLevels: 'Уровни',
        tabResize: 'Масштаб',
        tabFilter: 'Фильтр',
        levelsDesc: 'Настройте чёрную точку, гамму и белую точку по каналам.',
        resizeDesc: 'Масштабирование билинейной или ближайшей интерполяцией.',
        filterDesc: 'Применить свёрточное ядро 3×3 к одному или всем каналам.',
      },
      kernel: {
        title: 'Пользовательский фильтр',
        preset: 'Набор',
        matrix: 'Матрица ядра',
        channels: 'Применить к каналам',
        edgeHandling: 'Обработка краёв',
        processing: 'Обработка…',
        custom: 'Произвольное',
        open: 'Пользовательский фильтр…',
        presets: {
          identity: 'Тождественное',
          sharpen: 'Повышение резкости',
          gaussian: 'Фильтр Гаусса (3×3)',
          boxBlur: 'Прямоугольное размытие',
          prewittX: 'Прюитт X (горизонтальный)',
          prewittY: 'Прюитт Y (вертикальный)',
        },
        edge: {
          black: 'Заполнить чёрным',
          white: 'Заполнить белым',
          copy: 'Копировать край',
        },
      },
    },
  },
}

i18n.use(initReactI18next).init({
  resources,
  lng: language,
  fallbackLng: 'en',
  interpolation: { escapeValue: false },
})

// Экспорт экземпляра i18n для подключения в приложении
export default i18n
